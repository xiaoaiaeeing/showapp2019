...
public class WaveLoadingView extends View {
	...
	// This is incorrect/not recommended by Joshua Bloch in his book Effective
	// Java (2nd ed).
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		// The ANTI_ALIAS_FLAG bit AntiAliasing smooths out the edges of what is
		// being drawn,
		...
		// Init AmplitudeRatio
		float amplitudeRatioAttr = attributes.getFloat(R.styleable.WaveLoadingView_wlv_waveAmplitude,
				DEFAULT_AMPLITUDE_VALUE) / 1000;
		...
		mRoundRectangleXY = attributes.getInteger(R.styleable.WaveLoadingView_wlv_round_rectangle_x_and_y,
				DEFAULT_ROUND_RECTANGLE_X_AND_Y);

		// Init Triangle direction
		mTriangleDirection = attributes.getInteger(R.styleable.WaveLoadingView_wlv_triangle_direction,
		...
		mBorderPaint.setStrokeWidth(
				attributes.getDimension(R.styleable.WaveLoadingView_wlv_borderWidth, dp2px(DEFAULT_BORDER_WIDTH)));
		...
		mTopTitlePaint
				.setColor(attributes.getColor(R.styleable.WaveLoadingView_wlv_titleTopColor, DEFAULT_TITLE_COLOR));
		mTopTitlePaint.setStyle(Paint.Style.FILL);
		mTopTitlePaint.setAntiAlias(true);
		mTopTitlePaint.setTextSize(
				attributes.getDimension(R.styleable.WaveLoadingView_wlv_titleTopSize, sp2px(DEFAULT_TITLE_TOP_SIZE)));

		mTopTitleStrokePaint = new Paint();
		mTopTitleStrokePaint.setColor(
				attributes.getColor(R.styleable.WaveLoadingView_wlv_titleTopStrokeColor, DEFAULT_STROKE_COLOR));
		mTopTitleStrokePaint.setStrokeWidth(attributes.getDimension(R.styleable.WaveLoadingView_wlv_titleTopStrokeWidth,
		...
		mCenterTitlePaint
				.setColor(attributes.getColor(R.styleable.WaveLoadingView_wlv_titleCenterColor, DEFAULT_TITLE_COLOR));
		...
		mCenterTitleStrokePaint.setColor(
				attributes.getColor(R.styleable.WaveLoadingView_wlv_titleCenterStrokeColor, DEFAULT_STROKE_COLOR));
		mCenterTitleStrokePaint.setStrokeWidth(attributes.getDimension(
				R.styleable.WaveLoadingView_wlv_titleCenterStrokeWidth, dp2px(DEFAULT_TITLE_STROKE_WIDTH)));
		...
		mBottomTitlePaint
				.setColor(attributes.getColor(R.styleable.WaveLoadingView_wlv_titleBottomColor, DEFAULT_TITLE_COLOR));
		...
		mBottomTitleStrokePaint.setColor(
				attributes.getColor(R.styleable.WaveLoadingView_wlv_titleBottomStrokeColor, DEFAULT_STROKE_COLOR));
		mBottomTitleStrokePaint.setStrokeWidth(attributes.getDimension(
				R.styleable.WaveLoadingView_wlv_titleBottomStrokeWidth, dp2px(DEFAULT_TITLE_STROKE_WIDTH)));
	...
	public void onDraw(Canvas canvas) {
		...
		if (mWaveShader != null) {
			...
			// This decides the size(waveLengthRatio for width, amplitudeRatio
			// for height) of waves.
			...
			// This decides the start position(waveShiftRatio for x,
			// waterLevelRatio for y) of waves.
			...
			case 1:
				if (borderWidth > 0) {
					canvas.drawCircle(getWidth() / 2f, getHeight() / 2f, (getWidth() - borderWidth) / 2f - 1f,
							mBorderPaint);
				}
			...
			case 2:
				...
				canvas.drawRect(borderWidth, borderWidth, getWidth() - borderWidth, getHeight() - borderWidth,
						mWaveBgPaint);
				canvas.drawRect(borderWidth, borderWidth, getWidth() - borderWidth, getHeight() - borderWidth,
						mWavePaint);
				break;
			...
			case 3:
				if (mIsRoundRectangle) {
					if (borderWidth > 0) {
						RectF rect = new RectF(borderWidth / 2f, borderWidth / 2f, getWidth() - borderWidth / 2f - 0.5f,
			...
			if (!TextUtils.isEmpty(mCenterTitle)) {
				...
				canvas.drawText(mCenterTitle, (getWidth() - middle) / 2,
						getHeight() / 2 - ((mCenterTitleStrokePaint.descent() + mCenterTitleStrokePaint.ascent()) / 2),
			...
			if (!TextUtils.isEmpty(mBottomTitle)) {
				...
				canvas.drawText(mBottomTitle, (getWidth() - bottom) / 2,
						getHeight() * 8 / 10.0f
								- ((mBottomTitleStrokePaint.descent() + mBottomTitleStrokePaint.ascent()) / 2),
	...
	private void updateWaveShader() {
		// IllegalArgumentException: width and height must be > 0 while loading
		// Bitmap from View
	...
	public void setProgressValue(int progress) {
		...
		ObjectAnimator waterLevelAnim = ObjectAnimator.ofFloat(this, "waterLevelRatio", mWaterLevelRatio,
	...
	/**
	 * Sets the length of the animation. The default duration is 1000
	 * milliseconds.
	 *
	 * @param duration
