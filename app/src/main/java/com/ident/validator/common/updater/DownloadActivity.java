package com.ident.validator.common.updater;
...
import com.ident.validator.common.utils.RxUtils;
...
public class DownloadActivity extends AppCompatActivity implements View.OnClickListener {
	...
	private void startQuery() {
		mSubscriber = Observable.interval(1000, TimeUnit.MILLISECONDS)
				.compose(RxUtils.<Long>applyIOToMainThreadSchedulers()).subscribe(new Subscriber<Long>() {
	...
	public void onBackPressed() {
		if (force || status == STATUS_DOWNLOADING)
			return;
		super.onBackPressed();
	}
	...
	private void installApk(final Uri uri) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
			apkUri = FileProvider.getUriForFile(this, getPackageName() + ".provider", new File(uri.getPath()));
	...
	private void downloadApk() {
		...
		if (!path.exists() && !path.mkdirs()) {
			Toast.makeText(this, String.format(getString(R.string.app_updater_dir_not_found), path.getPath()),
